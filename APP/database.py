...
class DatabaseManager:
    def __init__(self, host='localhost', database='New_DB', user='postgres', password='Abhiraj@488', port=5432):
        self.conn_params = {
            'host': host,
            'dbname': database,
            'user': user,
    ...
    def _insert_sample_data(self):
        ...
        if cursor.fetchone()[0] == 0:
            ...
            for name, std_id, email, phone in sample_students:
                cursor.execute('''
                    INSERT INTO students (name, student_id, email, phone)
                    VALUES (%s, %s, %s, %s)
                    ON CONFLICT DO NOTHING
                ''', (name, std_id, email, phone))
        ...
        if cursor.fetchone()[0] == 0:
            sample_logs = [
                (1, datetime.now().strftime('%Y-%m-%d 09:15:00'), None, datetime.now().strftime('%Y-%m-%d'), 'Present', 'Face detected'),
                (2, datetime.now().strftime('%Y-%m-%d 08:50:00'), None, datetime.now().strftime('%Y-%m-%d'), 'Present', 'Face detected'),
                (3, datetime.now().strftime('%Y-%m-%d 09:45:00'), None, datetime.now().strftime('%Y-%m-%d'), 'Present', 'Face detected'),
                (5, datetime.now().strftime('%Y-%m-%d 09:05:00'), None, datetime.now().strftime('%Y-%m-%d'), 'Present', 'Face detected'),
                (6, datetime.now().strftime('%Y-%m-%d 10:10:00'), None, datetime.now().strftime('%Y-%m-%d'), 'Late', 'Arrived after 10 AM'),
            ]
            
            for std_id, check_in, check_out, date, status, notes in sample_logs:
                cursor.execute('''
                    INSERT INTO attendance_logs (student_id, check_in_time, check_out_time, date, status, notes)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    ON CONFLICT DO NOTHING
                ''', (std_id, check_in, check_out, date, status, notes))
    ...
    def get_attendance_stats(self):
        ...
        cursor.execute('''
            SELECT COUNT(*) as present FROM attendance_logs
            WHERE date = %s AND status = 'Present'
        ''', (today,))
        present = cursor.fetchone()[0]
        
        cursor.execute('''
            SELECT COUNT(*) as absent FROM attendance_logs
            WHERE date = %s AND status = 'Absent'
        ''', (today,))
        absent = cursor.fetchone()[0]
        
        cursor.execute('''
            SELECT COUNT(*) as late FROM attendance_logs
    ...
    def get_unique_faces_count(self):
        """Get count of unique faces detected"""
        conn = self.get_connection()
        cursor = conn.cursor()
